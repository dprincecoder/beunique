  ...
  const handleAddToBag = (item) => {
    if (size) {
      const bagItem = {
        ...
        price: item.price,
        size: size,
        image: item.product_url[0],
        category_slug: item.category_slug,
        item_slug: item.slug_name,
      };
    ...
    <section className="w-full relative inline-block group duration-300 mt-4">
      ...
      <section className="relative">
        ...
        <button
          ...
          onClick={() => handleSelectSizeOpen(item.id)}
        >
          Quick Add
        </button>
                  ...
                  if (size.selected) {
                    ...
                    return (
                      <section
                        ...
                        onClick={() => handleSize(size.id, item.id)}
                      >
                        {size.size}
                      </section>
                    );
      ...
      <section className="w-full flex items-start justify-between mt-5">
        <Link
          to={`/p?type=${item.category_slug}&item=${item.slug_name}`}
          className="w-full"
        >
